...
var min_diameter = 100;		//minimum diameter of a colony (in micrometers)
var	min_area = pow(min_diameter/2,2)*PI;
var min_circ = 0.15;			//minimum circularity of the detected objects (after watershedding if activated)
var max_diameter = 10000;
var	max_area = pow(max_diameter/2,2)*PI;
...
var radius = 1000;			//radius of the auto_local_threshold (in pixels)
var parameter1 = 25;		//threshold level (higher means more strict)
var	error = false;
var roi_color = "red";
var template = "template_normalized";
var dim = 1;				//start value of the dimension;

var nr_wells = 12;			//Number of wells
var nr_wells_x=0;
var nr_wells_y=0;
...
var pixel_size=31.75;		//pixel size in micrometers - TO DO: configure using DPI settings of the scanner
...
//		pixel_size = pixel_width;	//For now: Use the hardcoded value defined above.
		...
		if(automatic_detection == true) detect_wells();
		else {
...
//waitForUser("after subtraction");
	...
	run("Clear Outside");
	run("Analyze Particles...", "size="+min_area+"-"+max_area+" circularity="+min_circ+"-1.00 display clear summarize exclude show=[Bare Outlines]");
