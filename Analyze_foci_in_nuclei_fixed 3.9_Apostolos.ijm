 ...
 *  
 * Bram van den Broek, Netherlands Cancer Institute, 2012-2015
 * b.vd.broek@nki.nl
 * 
...
var large_dots=true;
var current_slice = 1;
var median_radius_nuclei = 3;	//Was set at 2!
var lower_limit = 0;
...
var alth_radius = 50;
var alth_parameter_1 = -2;
var bgsubtr_nuclei_segmentation = true;
...
var threshold_scaling_coeff = 0.5;	//Use a large negative number for no differential thresholding, 0.5 for the square root of the difference with the average threshold of all nuclei. 
...
var outliers_threshold = 0;		//threshold for foci outlier removal before retreiving median and stddev of nucleus
...
var sigma_large = 2*foci_diameter;	//size of Gaussian blur filter (large) for foci background subtraction
var sigma_small = 0.5;				//size of Gaussian blur filter (small) for foci background subtraction
//var sigma_small = 0.25*foci_diameter;	//size of Gaussian blur filter (small) for foci background subtraction
...
results_file = newdir+"\\results_foci.xls";
...
if (File.exists(config_file)) {
	...
	if (config_array.length==24 || config_array.length==22) {
		...
		shrink_nuclei = parseInt(config_array[7]);
		shrink_nuclei_size = parseFloat(config_array[8]);		
		manually_edit_nuclei = parseInt(config_array[9]);
		
		foci_diameter = parseFloat(config_array[10]);
		min_foci_size = parseFloat(config_array[11]);
		SNR = parseFloat(config_array[12]);
		foci_background_correct=parseInt(config_array[13]);
		manual_foci_threshold = config_array[14];

		choice_additional_nucleus_channel = parseInt(config_array[15]);
		ch_measure_additional = parseInt(config_array[16]);
		choice_exclude_nucleus_channel = parseInt(config_array[17]);
		ch_exclude = parseInt(config_array[18]);
		choice_exclude_threshold = config_array[19];
		exclude_threshold = parseInt(config_array[20]);

		verbose = parseInt(config_array[21]);

		if(config_array.length==24) nuclei_method = config_array[22];
		if(config_array.length==24) foci_projection_method = config_array[23];
	}
}
...
Dialog.show;
	...
	exclude_edges = Dialog.getCheckbox();			//6
	shrink_nuclei = Dialog.getCheckbox();			//7
	shrink_nuclei_size = Dialog.getNumber();		//8
	manually_edit_nuclei = Dialog.getCheckbox();	//9

	if(slices>1) foci_projection_method = Dialog.getChoice();//23	//only when it is a z-stack
	if (slices>1 && foci_projection_method=="best slice for every nucleus") focus_nuclei=true;
	foci_diameter = Dialog.getNumber();				//10
	min_foci_size = Dialog.getNumber();				//11
	SNR = Dialog.getNumber();						//12
	foci_background_correct=Dialog.getCheckbox();	//13
	manual_foci_threshold = Dialog.getNumber();		//14
	
	choice_additional_nucleus_channel = Dialog.getCheckbox();//15
	ch_measure_additional = Dialog.getNumber();		//16
	...
	choice_exclude_nucleus_channel = Dialog.getCheckbox();	//17
	ch_exclude = Dialog.getNumber();				//18
	...
	choice_exclude_threshold = Dialog.getChoice();	//19
	exclude_threshold = Dialog.getNumber();			//20

	verbose=Dialog.getCheckbox();					//21
...
if(analyze_all==true) {
	...
	run("32-bit");
	run("16-bit");	//get rid of weird DeltaVision calibration that is sometimes present (-32768)
	...
	results_file = newdir+"\\"+substring(file_name,0,extension_index)+"_results_foci.xls";	//name of results file
...
if(nuclei_found==true) {
	...
	run("Duplicate...", "title=C"+ch_foci+"-"+file_name);
	...
	var mean_intensity_additional_channel = newArray(roiManager("count"));
...
function segment_nuclei() {
	...
	selectWindow("segmented_nuclei");
	run("32-bit");
	run("Remove Outliers...", "radius="+foci_diameter+" threshold="+outliers_threshold+" which=Bright");
...
if(foci_background_correct == false) {
		...
		run("Remove Outliers...", "radius="+foci_diameter+" threshold="+outliers_threshold+" which=Bright slice");	//to get a better estimate of the stddev
		if(verbose == false) setBatchMode("hide");
}
...
//			run("Distance Map");
			...
			selectWindow("foci");
			roiManager("Select",i);
			setSelectionLocation(ROI_x[i], ROI_y[i]);
			roiManager("Update");
...
//		else threshold=maxOf(SNR*avg_stddev+background,manual_foci_threshold);
		...
		print("Diff: "+sqrt_diff+", Threshold "+i+1+": "+threshold);
//setBatchMode("show");
...
//waitForUser("detected foci "+i);
		...
		run("Properties...", "channels=[] slices=[] frames=[] unit="+unit+" pixel_width="+pw+" pixel_height="+ph+" voxel_depth="+pd+" origin=0,0");	//set to original units/values in stead of pixels
...
function display_merged_image() {
	...
	Stack.setActiveChannels("11000");	//do not show DAPI
...
function handle_results() {
	run("Clear Results");
	run("Input/Output...", "file=.xls use_file copy_row copy_column save_column");	//do not copy row numbers, create xls file
	for(i=0;i<roiManager("Count");i++) {
