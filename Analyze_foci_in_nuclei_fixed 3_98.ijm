 ...
 * - Added loical thresholding option in the dialog. It's becoming ugly now, but will all be much better in the next incarnation...
 */
...
var sigma_small = 0;				//size of Gaussian blur filter (small) for foci background subtraction
...
Dialog.create("Options");
	...
	*/
	...
	Dialog.addCheckbox("If automatic segmentation, use local thresholding (adjust parameters in the code)", auto_local_th_nuc);
...
sigma_large = 2*foci_diameter;	//size of Gaussian blur filter (large) for foci background subtraction
sigma_small = 0.1*foci_diameter;				//size of Gaussian blur filter (small) for foci background subtraction
...
//		else threshold=maxOf(SNR*avg_stddev+background,manual_foci_threshold);
		...
		if(verbose==true) print("Diff: "+sqrt_diff+", Threshold "+i+1+": "+threshold);
//setBatchMode("show");
