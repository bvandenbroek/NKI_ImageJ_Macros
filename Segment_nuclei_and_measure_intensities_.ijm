...
thickOutlines = true;			// Dilate the outlines, making them 3 pixels wide instead of 1 pixel)
...
//Create a band around each nucleus using CLIJ2
	...
	//Shrink nuclei
	if(shrinkSize > 0) {
		Ext.CLIJx_shrinkLabels(labelmap_nuclei_filtered, labelmap_final, shrinkSize, false);
		Ext.CLIJ2_release(labelmap_nuclei_filtered);
	}
	if(shrinkSize < 0) {
		Ext.CLIJx_extendLabelsWithMaximumRadius(labelmap_nuclei_filtered, labelmap_final, -shrinkSize);
		Ext.CLIJ2_release(labelmap_nuclei_filtered);
	}
	...
	if(saveImages == true) {
		...
		if(thickOutlines) {	
			Ext.CLIJx_extendLabelsWithMaximumRadius(labelmap_final_ordered, labelmap_final_ordered_extended, 1);
		}
		...
		Ext.CLIJ2_release(labelmap_edges_dilated);
